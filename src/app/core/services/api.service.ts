...
export class ApiService {
  ...
  features(): Observable<any> {
    return forkJoin(
      this.store.select(fromStore.selectTrack).pipe(
        first(),
        switchMap(tracks =>
          from(chunk(tracks.ids, 100)).pipe(
            ...
            scan((acc, features) => [...acc, ...((features as any).audio_features as any[])], [])
          )
        )
      )
    ).pipe(map(features => flatten(features).filter(feature => !!feature)))
  }
